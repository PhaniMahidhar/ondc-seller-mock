...
from mobility import mock_utils
from mobility.search_model import Status, OnStatus, OnCancel, Cancel
...
def getCountAndUpdateStatus(ondc_order_id: str):
    ...
    if (order.order.message.order.fulfillment.state.descriptor.code != 'RIDE_CANCELLED') and (order.order.message.order.fulfillment.state.descriptor.code != 'RIDE_ENDED'):
        if count > 4:
